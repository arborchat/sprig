module git.sr.ht/~whereswaldon/sprig

go 1.14

require (
	gioui.org v0.0.0-20201110145814-8e4940e6a85d
	gioui.org/cmd v0.0.0-20201110145814-8e4940e6a85d
	git.sr.ht/~athorp96/forest-ex v0.0.0-20201012012825-01012995abe1
	git.sr.ht/~whereswaldon/colorpicker v0.0.0-20201108174423-c10ee301fc9a
	git.sr.ht/~whereswaldon/events v0.0.0-20201109021706-d2e4d9a38c3e
	git.sr.ht/~whereswaldon/forest-go v0.0.0-20201031211815-0e51e2b5d99c
	git.sr.ht/~whereswaldon/haptic v0.0.0-20201108211324-fb980219e1df
	git.sr.ht/~whereswaldon/materials v0.0.0-20201108152851-4af89701ceb6
	git.sr.ht/~whereswaldon/niotify v0.0.4-0.20200831115722-2354c7f8372f
	git.sr.ht/~whereswaldon/scroll v0.0.0-20201108173905-080302d3f39d
	git.sr.ht/~whereswaldon/sprout-go v0.0.0-20201031213922-5dae1cbb80f6
	git.wow.st/gmp/jni v0.0.0-20200827154156-014cd5c7c4c0 // indirect
	github.com/pkg/profile v1.5.0
	golang.org/x/crypto v0.0.0-20201016220609-9e8e0b390897
	golang.org/x/exp v0.0.0-20201008143054-e3b2a7f2fdc7
	golang.org/x/sys v0.0.0-20201109165425-215b40eba54c // indirect
	golang.org/x/text v0.3.4 // indirect
	golang.org/x/tools v0.0.0-20201110124207-079ba7bd75cd // indirect
)

replace golang.org/x/crypto => github.com/ProtonMail/crypto v0.0.0-20200605105621-11f6ee2dd602
