module git.sr.ht/~whereswaldon/sprig

go 1.14

require (
	gioui.org v0.0.0-20201127082448-4c3de5f5d294
	gioui.org/cmd v0.0.0-20201127082448-4c3de5f5d294
	git.sr.ht/~athorp96/forest-ex v0.0.0-20201012012825-01012995abe1
	git.sr.ht/~whereswaldon/colorpicker v0.0.0-20201124194604-d17e8b43f2ee
	git.sr.ht/~whereswaldon/events v0.0.0-20201124194128-35df8d2cd3cb
	git.sr.ht/~whereswaldon/forest-go v0.0.0-20201031211815-0e51e2b5d99c
	git.sr.ht/~whereswaldon/haptic v0.0.0-20201129165311-8c26cd6b5e7f
	git.sr.ht/~whereswaldon/materials v0.0.0-20201124200008-26d49e910dc5
	git.sr.ht/~whereswaldon/niotify v0.0.4-0.20200831115722-2354c7f8372f
	git.sr.ht/~whereswaldon/scroll v0.0.0-20201124194830-0742e8213a01
	git.sr.ht/~whereswaldon/sprout-go v0.0.0-20201031213922-5dae1cbb80f6
	golang.org/x/crypto v0.0.0-20201124201722-c8d3bf9c5392
	golang.org/x/exp v0.0.0-20201008143054-e3b2a7f2fdc7
	golang.org/x/sys v0.0.0-20201126233918-771906719818 // indirect
	golang.org/x/tools v0.0.0-20201125231158-b5590deeca9b // indirect
)

replace golang.org/x/crypto => github.com/ProtonMail/crypto v0.0.0-20200605105621-11f6ee2dd602
